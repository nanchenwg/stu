...
public class RedissonLockAspect {
    ...
    @Pointcut("@annotation(com.seckill.common.annotation.DistributedLock)")
    ...
    public String getLockName(Method method, Object[] args) {
        ...
        throw new IllegalArgumentException("无法正确获取锁名称!");
    }
    ...
    public Object lock(ProceedingJoinPoint pjp, final String lockName, boolean fairLock) {
    	try {
    		redissonLockUtil.lock(lockName, fairLock);
    		return proceed(pjp);
		} catch (Exception e) {
			e.printStackTrace();
		}finally {
    ...
    public Object tryLock(ProceedingJoinPoint pjp, DistributedLock annotation, final String lockName, boolean fairLock) {
        ...
        try {
        	status = redissonLockUtil.tryLock(lockName, waitTime, leaseTime, timeUnit, fairLock);
        	if(status){
				return proceed(pjp);
			}
        } catch (Exception e) {
